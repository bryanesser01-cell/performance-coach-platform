...
def analyse_training(
    ...
    average_pace = mean(s.average_pace for s in sessions)

    hr_values = [s.average_hr for s in sessions if s.average_hr is not None]

    average_hr = mean(hr_values) if hr_values else 0

    rpe_values = [s.rpe for s in sessions if s.rpe is not None]

    average_rpe = mean(rpe_values) if rpe_values else 0

    total_training_load = sum(s.training_load or 0 for s in sessions)

    longest_run = max(s.distance for s in sessions)
    ...
    if average_rpe <= settings.TARGET_RPE:
        ...
        risks.append("Training intensity may be too high.")
    ...
    if total_sessions < settings.MIN_WEEKLY_SESSIONS:
        ...
        strengths.append("Training consistency is improving.")

    # Weekly Volume
    if total_distance < settings.MIN_WEEKLY_DISTANCE:
        risks.append("Weekly running volume is below target.")
        recommendations.append("Gradually increase weekly distance by 5–10%.")
    else:
        strengths.append("Weekly running volume is solid.")
    ...
    if risks:
        ...
        coach_comment = "Training is progressing well. Continue building consistently."
