...
def test_generate_running_workout():
    ...
    assert result["running_workout"]["type"] == "intervals"
...
def test_generate_strength_workout():
    ...
    assert result["exercises"][0]["name"] == "Squats"

    assert result["exercises"][0]["sets"] == 4
...
def test_build_daily_coach_message():
    ...
    assert result["decision"] == "TRAIN_MODERATE"
...
def test_recovery_recommendation():
    ...
    assert result["sessions"][0]["session_type"] == "recovery"
