from api.services.ai_coach_live_route_service import (
    handle_chat_route,
    handle_conversation_route,
    validate_live_route_response,
)


def test_handle_conversation_route():

    result = handle_conversation_route(
        athlete_id=1,
        question="Should I train today?",
        response={
            "decision": "REDUCE_TRAINING",
            "confidence": 90,
            "outcome": "positive",
        },
    )

    assert result["success"] is True

    assert result["data"]["learning_applied"] is True


def test_handle_chat_route():

    result = handle_chat_route(
        athlete_id=1,
        question="Should I recover?",
        response={
            "decision": "RECOVERY",
            "confidence": 85,
            "outcome": "positive",
        },
    )

    assert result["success"] is True

    assert result["data"]["learning_applied"] is True


def test_validate_live_route_response():

    result = validate_live_route_response(
        {
            "success": True,
            "data": {
                "learning_applied": True,
            },
        },
    )

    assert result["valid"] is True
