...
def test_build_workout_response():
    ...
    assert response["title"] == "Next Training Session"

    assert "400m" in response["session"]

    assert response["effort"] == "8-9/10"
...
def test_build_race_response():
    ...
    assert response["title"] == "Race Strategy"
...
def test_build_explanation_response():
    ...
    context = {
        "term": "threshold",
        "explanation": ("Controlled hard running"),
        "effort": "7/10",
    ...
    assert response["title"] == "Training Explanation"

    assert response["effort"] == "7/10"
...
def test_response_to_text():
    ...
    assert "Coach Advice" in text

    assert "Keep progressing" in text
